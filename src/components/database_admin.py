    ...
    with col2:
        ...
        if backup_files:
            ...
            selected_backup = st.selectbox(
                "Select backup to restore", options=list(backup_options.keys()), format_func=lambda x: backup_options[x]
            )
    ...
    with col2:
        ...
        if st.button("Download Database File"):
            try:
                ...
                st.download_button(
                    label="Download SQLite Database", data=db_data, file_name=filename, mime="application/x-sqlite3"
                )
            except Exception as e:
