...
def render_dashboard(questions: List[TMMiQuestion]):
    ...
    org_options = {org["id"]: f"{org['name']} ({org['status']})" for org in organizations}
...
def render_maturity_trend(assessments: List, questions: List[TMMiQuestion], selected_org_id: int = None):
    ...
    fig.add_trace(
        go.Bar(x=df["Date"], y=df["TMMi Level"], name="TMMi Level", marker_color="#ff7f0e", opacity=0.6, yaxis="y2")
    )
    ...
    fig.update_layout(
        ...
        yaxis=dict(title="Compliance Percentage", side="left", range=[0, 100]),
        yaxis2=dict(title="TMMi Level", side="right", overlaying="y", range=[1, 6], dtick=1),
        hovermode="x unified",
        height=400,
    )
...
def render_process_area_compliance(summary: Dict):
    ...
    fig.update_layout(height=max(300, len(areas) * 40), xaxis=dict(range=[0, 100]), showlegend=False)
...
def render_evidence_coverage(summary: Dict):
    ...
    fig.update_layout(
        ...
        height=300,
        showlegend=True,
        annotations=[dict(text=f"{evidence_data['percentage']:.1f}%", x=0.5, y=0.5, font_size=20, showarrow=False)],
    )

    st.plotly_chart(fig, use_container_width=True)

    st.caption(
        f"{evidence_data['with_evidence']} out of {evidence_data['total_answers']} " f"answers have supporting evidence"
    )
...
def render_gap_analysis(summary: Dict):
    ...
    with col1:
        priority_filter = st.selectbox("Priority Filter", options=["All", "High", "Medium", "Low"], index=0)

    with col2:
        level_filter = st.selectbox("Level Filter", options=["All"] + [f"Level {i}" for i in range(2, 6)], index=0)

    with col3:
        answer_filter = st.selectbox("Answer Filter", options=["All", "No", "Partial", "Not Answered"], index=0)
    ...
    for i, gap in enumerate(gaps, 1):
        with st.expander(f"Gap {i}: {gap['process_area']} - Level {gap['level']}"):
            ...
            with col2:
                ...
                st.markdown(
                    f'<span class="{priority_class}">Priority: {gap["importance"]}</span>', unsafe_allow_html=True
                )
...
def render_level_breakdown():
    ...
    for level in sorted([2, 3, 4, 5]):
        if level in level_compliance:
            ...
            with st.expander(f"Level {level} - {level_names[level]}", expanded=(level == summary["current_level"])):
